...
import { Upload, X, Smartphone, Layout, Image as ImageIcon, ArrowRight, ArrowLeft, Grid, Download, Loader, PaintBucket, Layers, SplitSquareHorizontal } from 'lucide-react';

const Photogrid = () => {
  // --- State ---
  const [images, setImages] = useState([
    "https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=800&auto=format&fit=crop&q=60",
    "https://images.unsplash.com/photo-1762764919450-560fd6515192?w=600&auto=format&fit=crop&q=60",
  ...
  // 400% Width (4320px) x 100% Height (1350px)
  const layouts = {
    seamless: {
      name: "Seamless Flow",
  ...
  const loadImage = (src) => {
    return new Promise((resolve) => {
      const img = new Image();
      // Only set crossOrigin if it's NOT a data URI (base64)
      ...
      img.onerror = () => {
        ...
        resolve(null); // Resolve null so we can skip instead of crashing
      };
    });
  };
    ...
    <div className="min-h-screen bg-neutral-900 text-white font-sans selection:bg-pink-500 selection:text-white pb-20">
      ...
      <main className="max-w-6xl mx-auto px-4 mt-8">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-12">
          ...
          {/* 2. Layouts */}
          <div className="lg:col-span-6 bg-neutral-800/50 p-5 rounded-2xl border border-neutral-800">
            <h2 className="text-sm uppercase tracking-wider text-neutral-400 font-semibold mb-4 flex items-center gap-2">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3">
              {Object.entries(layouts).map(([key, layout]) => (
                <button
                  ...
                  className={`text-left p-3 rounded-xl border transition-all flex flex-col justify-between group h-28 relative overflow-hidden ${activeLayout === key ? 'bg-neutral-700 border-pink-500/50' : 'bg-neutral-900 border-neutral-700 hover:border-neutral-600'}`}
                >
                  <div className="z-10 relative">
                    <div className="font-medium text-white group-hover:text-pink-400 transition-colors text-xs">{layout.name}</div>
                    <div className="text-[10px] text-neutral-400 mt-1 leading-tight">{layout.description}</div>
                  </div>
                  {activeLayout === key && (
          ...
          <div className="lg:col-span-3 bg-neutral-800/50 p-5 rounded-2xl border border-neutral-800 h-fit">
            ...
            <div className="grid grid-cols-3 bg-neutral-900 p-1 rounded-lg mb-4">
              ...
              <button onClick={() => setBgMode('two-images')} className={`flex-1 py-1 text-[10px] font-medium rounded ${bgMode === 'image' ? 'bg-neutral-700 text-white' : 'text-neutral-500'}`}><ImageIcon size={12} className="inline mr-1" /> Image</button>
              <button onClick={() => setBgMode('split')} className={`mt-1 flex-1 py-1 text-[10px] font-medium rounded ${bgMode === 'split' ? 'bg-neutral-700 text-white' : 'text-neutral-500'}`}><SplitSquareHorizontal size={12} className="inline mr-1" /> Split</button>
            </div>
