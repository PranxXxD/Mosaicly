...
import { Upload, X, Smartphone, Layout, Image as ImageIcon, ArrowRight, ArrowLeft, Grid, Download, Loader, PaintBucket, Layers, SplitSquareHorizontal, Sparkles, Wand2, TowerControl, ImagePlusIcon, Images } from 'lucide-react';

const Photogrid = () => {
  const apiKey = import.meta.env.VITE_API_KEY; // API Key injected at runtime

  const imgUrl = ["https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=800&auto=format&fit=crop&q=60",
...
...imgUrl
  ...
  const handleImageUpload = (index, e) => {
    ...
    if (files.length > 1) {
      // ✅ If multiple files selected, append all to images
      const newFiles = files.map(file => URL.createObjectURL(file));
      setImages(prev => [...prev, ...newFiles]);
    } else {
      // ✅ If single file, replace image at given index
      const file = files[0];
      if (file) {
  ...
  //   try {
  //     const systemPrompt = `instruction": "Generate a JSON layout for an Instagram carousel as per the number mentioned in the user prompt arranged in a panoramic strip (400% width, 100% height). Each slide should contain one creative collage item inspired by the user prompt.",
  // "rules": {
  ...
  //     ]
  //   },
  //   "itemsCount": "array MUST have a count equal to the number mentioned in the user prompt (default = 4 if not specified).",
  //   "placement": "Ensure items are distributed across slides: slide 1 (0-90%), slide 2 (>100%), slide 3 (>200%), slide 4 (>300%)",
  //   "alignment": "Each item should have random but visually balanced top, width, height, zIndex, and rotate values."
  //   "creativity": "Design should reflect the user prompt dynamically with varied positions, rotations, and sizes for a collage effect"
  // },
  // "userPrompt": "${aiPrompt}" `;
